...
핵심 변경 사항 (R 대비):
  ...
  - 관측소 ID(asos108 등)에 해당하는 개별 CSV 파일에서 일단위 기상 자료 로드
  - 결측·이상값 처리 포함

Station-Info.csv 컬럼:
    Lon, Lat, Elev, ID, Ename, SYear

관측소 CSV 컬럼 (asos{ID}.csv):
    Year, Mon, Day, prec, tmax, tmin, wspd, rhum, rsds, shine, cloud, tavg
"""
...
def load_obs_csv(filepath: str, syear: int, eyear: int) -> pd.DataFrame:
    """단일 관측소 CSV 파일을 읽어 기간 필터링 후 반환.

    예상 컬럼: Year, Mon, Day, prec, tmax, tmin, [tavg, ...]
    """
    df = pd.read_csv(filepath)
    df.columns = df.columns.str.strip()

    # 컬럼명 정규화 (원본 R 코드는 Pcp.mm., Tmax.c. 형식도 지원)
    rename_map = {
        "Mon": "Month",
        "Pcp.mm.": "prec",
        "Tmax.c.": "tmax",
    ...
    if "Month" not in df.columns and "Mon" in df.columns:
